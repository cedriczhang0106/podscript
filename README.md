...
> **▶ 最简单的用法：网页版（推荐）**
> 装好依赖后跑 `uvicorn backend.app:app --port 8000`，浏览器打开 **http://127.0.0.1:8000**——
> 粘链接 → 点「转写」→ 看摘要 / 大纲 / 逐字稿 → 一键导出。界面化操作，新手首选，比命令行直观得多。
> （网页在你**本地**运行、数据不出本机，因此没有公网地址；完整步骤见下「安装」+「用法 A · 网页版」。）
